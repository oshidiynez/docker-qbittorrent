...
# :: BUILD THE IMAGE
  ...
  RUN set -ex; \
    sed -i 's|const auto USER_AGENT = QStringLiteral("qBittorrent/" QBT_VERSION_2);|const char *envUserAgent = getenv("QBITTORRENT_USER_AGENT");\ \n    const auto USER_AGENT = envUserAgent ? QString::fromUtf8(envUserAgent) : QStringLiteral("qBittorrent/" QBT_VERSION_2);|' /qBittorrent/src/base/bittorrent/sessionimpl.cpp; \
